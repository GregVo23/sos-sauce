<template>
    ...
    <div :class="[dark ? 'bg-gray-600' : 'bg-white','xl:flex pt-6']">
        ...
        <div class="xl:w-1/2">
        <transition name="filterSearch" enter-active-class="transition ease-out duration-1000" enter-from-class="transform opacity-0 scale-95" :enter-to-class="[dark ? 'opacity-100' : 'opacity-70','transform scale-100']" leave-active-class="transition ease-in duration-1000" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <img v-if="charged" :src="URL + 'storage/meals/' + meal.picture" :alt="meal.name" :class="[dark ? 'opacity-70' : 'opacity-100', 'xl:rounded-tr-3xl w-full h-full object-center object-cover']" />
        </transition>
        </div>
        ...
        <div class="xl:w-1/2 p-8">
            ...
            <p :class="[dark ? 'text-white' : 'text-gray-800', 'mt-10']">{{ meal.number }} personne{{ (meal.number > 1) ? 's' :'' }}</p>
        ...
        <div :class="[dark ? 'bg-gray-600' : 'bg-white','xl:flex justify-around p-8']">
            <div class="mt-16 space-y-16">
                ...
                <div :class="[featureIdx % 2 === 0 ? 'lg:col-start-6 xl:col-start-5' : 'lg:col-start-1', 'flex-auto lg:row-start-1 lg:col-span-7 xl:col-span-8']">
                    <div class="aspect-w-5 aspect-h-2 rounded-3xl bg-gray-500 overflow-hidden">
                        <img :src="URL + 'storage/meals/' + meal.picture" :alt="meal.name" :class="[dark ? 'opacity-70' : 'opacity-100', 'object-center object-cover']" />
                    </div>
                </div>
                </div>
            </div>
