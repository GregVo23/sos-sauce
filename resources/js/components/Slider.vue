<template>
        ...
        <div class="flex justify-between w-full">
            <div>
                <button @click="previous()" class="h-16 w-16 absolute left-10 top-1/2 z-50 bg-gray-100 hover:bg-red-600 hover:text-white text-gray-800 font-bold py-2 px-4 rounded-full">
                    Prev
                </button>
            </div>
            <div>
                <button @click="next()" class="h-16 w-16 absolute right-10 top-1/2 z-50 bg-gray-100 hover:bg-red-600 hover:text-white text-gray-800 font-bold py-2 px-4 rounded-full">
                    Next
                </button>
            </div>
        </div>
        <div class="slider">
            <div class="container-slide" :style="{transform: `translateX(${index}px)`, transition:`${transition}`}">
                <div v-for="meal in meals" :key="meal.id" class="div-slider" :style="`width:100vw; height:100vh; background-image:url(http://localhost:8000/storage/meals/${meal.picture}); background-size: cover;`">
                    ...
                    <p class="mt-6 mx-auto px-12 text-2xl text-white text-center truncate">{{ meal.description }}</p>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        next(){
            if(Math.abs(this.index/window.screen.availWidth)+1 === this.meals.length) {
                this.transition = "";
                this.index = 0
            } else {
                ...
                let image = document.querySelector("#app > section > div > div > img:nth-child(" + x + ")");
                console.log("h "+x);
